  ...
  <div>
    <div class="grid sm:grid-cols-12 <sm:grid-cols-8 ">
      <div class='text-center' >
        <p class='text-orange-500' >{{ convertDate(departure.flight.time.scheduled.arrival) }}</p>
        <svg v-if='shown' xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" role="img" class=" text-gray-500 m-auto h-6 w-6 transform rotate-90 " width="32" height="32" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24"><path d="M23 12l-4 4v-3H6.83A2.99 2.99 0 0 1 4 15a3 3 0 0 1-3-3a3 3 0 0 1 3-3c1.31 0 2.42.83 2.83 2H19V8l4 4z" fill="currentColor"></path></svg>

      </div>
      <div class="sm:col-span-8 md:col-span-7 <sm:col-span-4 ">
        <p class="md:text-xl text-gray-100 font-semibold">{{ departure.flight.airport.destination.position.region.city }}</p>
        <p v-if='departure.flight.airline' >{{ departure.flight.airline.name }} &mdash; {{ departure.flight.identification.number.default }}</p> 
        <p v-else >Unknown</p>       
      </div>
      <div class="m-auto <md:hidden ">
        <p>{{ departure.flight.airport.destination.position.country.code }}</p>
      </div> 
      <div  class=" col-span-2  text-center flex my-auto"  >
        <p class="my-auto w-full bg-dark-900  mx-auto px-2 rounded-md " :class="{ estimated: isEstimated(departure.flight.status.text), canceled: isCanceled(departure.flight.status.text), delayed: isDelayed(departure.flight.status.text), landed: isLanded(departure.flight.status.text) }" >{{ departure.flight.status.text }}</p>
      </div>
    ...
    <div v-if="shown" class="grid sm:grid-cols-12 <sm:grid-cols-8 ">
      <div class='text-center text-gray-500 ' >
        <p> {{ convertDate(departure.flight.time.scheduled.departure) }} </p>
      </div>
      <div class="flex align-bottom sm:col-span-8 <sm:col-span-4 -ml-2">
        <div class="flex bg-dark-900 rounded-md px-2">
          <p class="flex overflow-hidden overflow-ellipsis ...">{{ departure.flight.airport.destination.name }} ({{ departure.flight.airport.destination.code.iata }}), {{ departure.flight.airport.destination.position.country.name }}</p>
          <span class="my-auto">
            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" role="img" class="h-6 w-6" width="32" height="32" preserveAspectRatio="xMidYMid meet" viewBox="0 0 24 24"><path d="M5 17.59L15.59 7H9V5h10v10h-2V8.41L6.41 19L5 17.59z" fill="currentColor"></path></svg>
          </span>
        </div>
      </div>
      <div class="flex bg-dark-900 col-span-2 rounded-md">
        <div class="mx-auto flex my-auto">
          ...
          <p>{{ departure.flight.aircraft.model.code }}</p>
        </div>
      </div>
    </div>
    <hr class="opacity-10" />
